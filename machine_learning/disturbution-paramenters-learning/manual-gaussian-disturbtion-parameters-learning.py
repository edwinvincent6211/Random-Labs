...
plt.scatter(X, Y, s=3, color='hotpink')  #plot the points
...
class Autoencoder(Model):
    def __init__(self, latent_dim):
        ...
        self.decoder = tf.keras.Sequential([
            layers.Dense(NN_DIMENSION, activation='sigmoid'),
            layers.Dense(NN_DIMENSION, activation='sigmoid'),
            layers.Dense(NN_DIMENSION, activation='sigmoid'),
...
for i in range(0, len(X)):
    y = autoencoder.decoder(np.array([[X[0]]])).numpy()
    YY.append(y)
...
# fig = plt.figure(1)  #identifies the figure
# plt.title("gassian distribution", fontsize='10')  #title
plt.scatter(X, YY, s=3)  #plot the points
plt.xlabel("X", fontsize='10', color='blue')  #adds a label in the x axis
plt.grid()  #shows a grid under the plot
plt.show()
